...
class hDQN():
    ...
    def check_goal(self, goal, state):
        # goal and state are arrays
        goal_array = my_hdqn_learning.one_hot_goal(goal)[0]
        return np.all(goal_array == state[:len(goal_array)])
    
    def check_goal_lines(self, goal, extrinsic_reward):
        # goal and state are arrays
